    ...
    <aside className="w-80 border-r border-slate-800 flex flex-col bg-slate-950 p-6 space-y-6 overflow-y-auto">
      ...
      <section className="bg-slate-900/50 p-4 rounded-xl border border-slate-800 space-y-3">
        ...
        <div className="flex items-center gap-3">
          ...
          <input type="range" min="0" max="12" value={currentStyle.outlineWidth} onChange={(e) => handleStyleChange({ outlineWidth: Number(e.target.value) })} className="flex-1" />
        </div>
        <div className="flex items-center gap-3">
      ...
      <section className="bg-slate-900/50 p-4 rounded-xl border border-slate-800 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-2">
          <select value={currentStyle.alignment} onChange={(e) => handleStyleChange({ alignment: e.target.value as Alignment })} className="h-8 bg-slate-950 border border-slate-700 rounded text-[10px] px-1">
            <option value="left">Left</option><option value="center">Center</option><option value="right">Right</option>
          </select>
          <select value={currentStyle.verticalAlign} onChange={(e) => handleStyleChange({ verticalAlign: e.target.value as VerticalAlignment })} className="h-8 bg-slate-950 border border-slate-700 rounded text-[10px] px-1">
            <option value="top">Top</option><option value="middle">Middle</option><option value="bottom">Bottom</option>
          </select>
        </div>
        <div className="grid grid-cols-2 gap-2">
          <input type="number" value={currentStyle.padding} onChange={(e) => handleStyleChange({ padding: Number(e.target.value) })} className="h-8 bg-slate-950 border border-slate-700 rounded text-[10px] px-2" placeholder="Safe Area" title="Safe Area (Padding)" />
          <select value={currentStyle.boxType} onChange={(e) => handleStyleChange({ boxType: e.target.value as BoxType })} className="h-8 bg-slate-950 border border-slate-700 rounded text-[10px] px-1">
      ...
      <section>
        ...
        <textarea placeholder="Page 1 - Dialog..." className="w-full h-32 bg-slate-900 border border-slate-800 rounded-lg p-3 text-xs outline-none" onBlur={(e) => onTextImport(e.target.value)} />
      </section>
    </aside>
  );
};
