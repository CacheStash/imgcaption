...
const App: React.FC = () => {
    ...
    setState(prev => ({
      ...
      pages: prev.pages.map((page, index) => {
        ...
        if (parsedData[pageNum]) {
          ...
          const newObjects = parsedData[pageNum].map(txt => createDefaultTextObject(txt, style));
  ...
  const addTextManually = useCallback((pageId: string) => {
    ...
    setState(prev => ({
      ...prev,
      pages: prev.pages.map(p => (p.id === pageId) ? { 
        ...p, textObjects: [...p.textObjects, createDefaultTextObject("New Dialogue", p.isLocalStyle && p.localStyle ? p.localStyle : prev.globalStyle)] 
      } : p),
      selectedTextId: null 
    }));
  }, [recordHistory]);
  ...
  const toggleLocalSettings = useCallback((pageId: string) => {
    ...
    setState(prev => ({
      ...prev,
      pages: prev.pages.map(p => (p.id === pageId) ? {
        ...p, isLocalStyle: !p.isLocalStyle, localStyle: !p.isLocalStyle ? JSON.parse(JSON.stringify(prev.globalStyle)) : undefined
      } : p)
    }));
  }, [recordHistory]);
    ...
    return new Promise<string>((resolve) => {
      fabric.Image.fromURL(page.imageUrl, (img: any) => {
        ...
        (page.masks || []).forEach(m => {
          staticCanvas.add(new fabric.Rect({ left: (m.x/100)*oW, top: (m.y/100)*oH, width: m.width*scale, height: m.height*scale, fill: m.fill, originX: 'center', originY: 'center' }));
        });
        page.textObjects.forEach((obj) => {
          const content = cleanText(obj.originalText, state.hideLabels);
          const fWidth = state.importMode === 'full' ? oW - ((obj.paddingLeft + obj.paddingRight + 40)*scale) : obj.width*scale;
          const fObj = new fabric.Textbox(content, { width: fWidth, fontSize: obj.fontSize*scale, fill: obj.color, textAlign: 'center', originX: 'center', originY: 'center', stroke: obj.outlineColor, strokeWidth: obj.outlineWidth*scale, fontFamily: obj.fontFamily || 'Inter', strokeUniform: true, paintFirst: 'stroke', shadow: new fabric.Shadow({ color: obj.glowColor, blur: obj.glowBlur*scale, opacity: obj.glowOpacity }) });
          fObj.setCoords();
          fObj.set({ left: Math.max((obj.paddingLeft*scale)+(fWidth/2), Math.min(oW-(obj.paddingRight*scale)-(fWidth/2), (obj.x/100)*oW)), top: Math.max((obj.paddingTop*scale)+(fObj.height/2), Math.min(oH-(obj.paddingBottom*scale)-(fObj.height/2), (obj.y/100)*oH)) });
  ...
  const goToPrevPage = useCallback(() => {
    setState(prev => {
      ...
      if (idx > 0) {
        const id = prev.pages[idx-1].id;
        return { ...prev, selectedPageId: id, selectedTextId: null, pages: prev.pages.map(p => (p.id === id && p.isLocalStyle === undefined) ? { ...p, isLocalStyle: true, localStyle: JSON.parse(JSON.stringify(prev.globalStyle)) } : p) };
      }
      return prev;
    });
  }, []);
    ...
    <div className="flex h-screen bg-slate-950 text-slate-100 overflow-hidden">
      ...
      <main className="flex-1 relative overflow-auto bg-slate-900 p-8">
        {state.pages.length === 0 ? (
          ...
          <>
            {state.isGalleryView ? (
              <Gallery pages={state.pages} hideLabels={state.hideLabels} onSelectPage={(id) => setState(prev => ({ ...prev, selectedPageId: id, isGalleryView: false, pages: prev.pages.map(p => (p.id === id && p.isLocalStyle === undefined) ? { ...p, isLocalStyle: true, localStyle: JSON.parse(JSON.stringify(prev.globalStyle)) } : p) }))} />
            ) : (
              <div className="h-full flex flex-col items-center">
                ...
                {selectedPage && (
                  <Editor key={selectedPage.id} page={selectedPage} hideLabels={state.hideLabels} importMode={state.importMode} 
