import { TextObject, TextStyle, BubbleObject } from './types'; // FIX: Path ke folder yang sama
...
// FIX: Logika hapus label yang lebih akurat sesuai permintaanmu
export const cleanText = (text: string, hideLabels: boolean): string => {
  if (!hideLabels) return text.trim();
  return text
    .replace(/Page\s+\d+\s*-\s*/gi, '') // Hapus "Page 80 - "
    .replace(/(?:^|,\s*)[^:]+:\s*/g, (match) => match.startsWith(',') ? ', ' : '') // Hapus "Lara: " atau ", Otto: "
    .trim();
};
...
export const parseRawText = (text: string): Record<number, string[]> => {
  ...
  for (let i = 1; i < sections.length; i += 2) {
    ...
    const dialogues = content.split(/,\s*(?=[^:]+:)/).map(d => d.trim()).filter(d => d.length > 0);
